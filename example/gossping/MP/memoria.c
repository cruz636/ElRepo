...
void *start_seeding(struct seedLog *seed);
...
void *connectToSeeds(int size){
  ...
  sleep(sleepTime);
	...
	for(int v = 1; v < size;v++){ //its start from 1 because the first ip&port its our adress
		struct seedLog seed;
		strcpy(ipSeed[v],seed.IP);
		strcpy(portSeed[v],seed.PORT);
    start_seeding(&seed);
	}
...
void *start_seeding(struct seedLog *seed){
  ...
  log_info(file_log,"estableciendo conexion");
	...
	printf("[+]Connecting to : %s",seed->IP);
	printf(":%s\n",seed->PORT);

  int socketClient = establecerConexion(seed->IP,seed->PORT,file_log,&control);
  if (control != 0) {
