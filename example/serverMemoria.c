...
void cargarListaMetaData(t_list *listaMetaData) {
...
void atenderMjs(header *req, void *buffer, int sockeClient) {
    ...
    switch (req->codigo) {
        ...
        case 6:{
            ...
            cargarListaMetaData(listametadata);
            void *buffer_LM = serealizarListaMetaData(listametadata, &sizebuffer);
            response.letra = 'M';
            response.codigo = 6;
            response.sizeData = sizebuffer;
            bufferMensaje = createMessage(&response, buffer_LM);
            enviar_message(sockeClient, bufferMensaje, file_log, &control);
            break;
        }
    }
